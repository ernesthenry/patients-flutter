...
import 'package:spouts_inventory_odoo/app/pages/shiptocustomer.dart';
import 'package:spouts_inventory_odoo/app/pages/shiptosales_old.dart';
import 'package:spouts_inventory_odoo/app/pages/stocktaking.dart';
  ...
  _refreshData() async {
    isConnected().then((isInternet) async {
      if (isInternet) {
        ...
        await _getEmployeeData(_userId);
          ...
          (OdooResponse res) {
            if (!res.hasError()) {
              setState(() {
                ...
                _employeeId = result["company_user_code"] is! bool
                    ? result["company_user_code"]
                    : "000";
                _isTeamLeader = result["team_leader"];
                _employeeLocationId =
                    result["location_id"] is! bool ? result["location_id"] : [];
                _displayEmployeeId = _employeeId?.substring(
                    (_employeeId.length - 3).clamp(0, _employeeId.length));
              ...
              // preference.setString("employeeid", _employeeId);
              // preference.setString("displayemployeeid", _displayEmployeeId);
            } else {
              print(res.getError());
...
//GET SALES OFFICERS
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: scaffoldKey,
      appBar: AppBar(
        title: Text(_firstName + " - " + _displayEmployeeId.toString()),
        actions: [
          IconButton(
              onPressed: () {
                _refreshData();
