...
class DeliveryDetailsOld extends StatefulWidget {
  DeliveryDetailsOld({this.data});

  final data;

  @override
  _DeliveryDetailsOldState createState() => _DeliveryDetailsOldState();
}

class _DeliveryDetailsOldState extends Base<DeliveryDetailsOld>
        ...
        return AlertDialog(
          ...
          content: new Row(
            ...
            children: [
              ...
              new Text("Validating stock move ....."),
            ],
          ),
        );
      },
  ...
  _saveStockMoveLines() {
    ...
    for (var i in _stockMoveLines) {
      ...
      odoo.create(Strings.stock_move_lines, {
        // "move_id": i.move_id[0],
        "picking_id": _stockPicking.id,
        "product_id": i.product_id[0],
        "lot_id": i.lot_id[0],
        "qty_done": i.qty_done,
        "product_uom_qty": i.qty_done,
        "product_uom_id": i.product_uom_id[0],
        "location_id": _stockPicking.location_id[0],
        "location_dest_id": _customerLocationId,
      }).then(
        (OdooResponse res) {
          if (!res.hasError()) {
                                    ...
                                    cells: <DataCell>[
                                      DataCell(Text(element.product_id[1]
                                          ...
                                          element.product_uom_id[1]
                                              .toString())), //Extracting from Map element the value
                                      DataCell(
                                          Text(element.lot_id[1].toString())),
                                      DataCell(
